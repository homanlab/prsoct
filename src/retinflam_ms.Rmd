...
shorttitle        : "Neuroinflammatory pathway and retinal atrophy" 
...
tab1_capt <- "Sample characteristics highlighting the distribution of key demographic
and clinical characteristics among the study participants. It includes gender, medical
conditions (diabetes mellitus and hypertension), smoking status, alcohol consumption
habits, age, body mass index (BMI), socioeconomic status (Townsend index), and macular
thickness measurements for both eyes. The sample size (N) is provided for each category,
with means and standard deviations (SD) given for continuous variables where applicable. "
...
## Associations between genetic schizophrenia risk and retina
...
## Neuroinflammatory pathway-specific association
...
colnames(df_gcipl)[1] <- ""
```
## The mediating role of C-reactive protein levels 
...
# Methods
## Base data: schizophrenia GWAS 2022
...
## Pathway-based polygenic risk scores and analysis
...
## Assessments of optical coherence tomography data
...
## Data availability
The summary statistics for the schizophrenia GWAS are publicly available 
at https://pgc.unc.edu/for-researchers/download-results, while the discovery 
data used in the study is based on the GRCh37/hg19 human genome assembly, 
which can be accessed through the NCBI Datasets resource at https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_000001405.13/.  
The rest of the data utilized in this study is publicly available 
at the UK Biobank (http://www.ukbiobank.ac.uk/) and was accessed via 
application number 102266. 

## Code availability
All data analyses were performed between June 2023 and
Feburary 2024 using Python (version 3.9.12) and R (version 4.0.5).
The polygenic risk scores were computed using PRSet (version 2.3.5) [@Choi2020].
The manuscript was produced with the R packages rmarkdown
(version `r packageVersion("rmarkdown")`) [@Allaire2014]; 
MASS (version `r packageVersion("MASS")`) [@Ripley2009]; 
SjPlot (version `r packageVersion("SjPlot")`) [@Ldecke2013]; 
represearch (https://github.com/phoman/represearch/); 
kableExtra (version `r packageVersion("kableExtra")`) [@Zhu2017]; 
knitr (version `r packageVersion("knitr")`) [@Xie2012];
papaja (version `r packageVersion("papaja")`) [@Aust2022];
matplotlib (version 3.7.3) [@Hunter2007];
seaborn (version 0.11.2) [@Waskom2021]; and
scipy (version 1.9.1).[@Virtanen2020]
...
## Signaling-pathway-specific analyses
A PRSet p-value threshold was set 
